...
export SAVE_DIR="${SAVE_DIR:-/home/kxr/zoedepth_kitti_brrh_noclsloss_scale0p24_256x512_bs4_workers8_5ep_checkpoints}"
export LOG_FILE="${LOG_FILE:-${REPO_DIR}/logs/kitti_brrh_noclsloss_scale0p24_256x512_bs4_workers8_5ep.log}"
export DONE_FILE="${DONE_FILE:-${REPO_DIR}/logs/kitti_brrh_noclsloss_scale0p24_256x512_bs4_workers8_5ep.done}"
...
if [[ -f "${DONE_FILE}" ]]; then
  echo "===== SKIP BRRH no-cls-loss scale0.24; done marker exists: ${DONE_FILE} ====="
  exit 0
fi

echo "===== START BRRH no-cls-loss scale0.24 epochs=${EPOCHS} checkpoint=${CHECKPOINT} $(date '+%F %T %Z') ====="
bash scripts/run_kitti_train_once.sh
echo "===== DONE BRRH no-cls-loss scale0.24 $(date '+%F %T %Z') ====="
