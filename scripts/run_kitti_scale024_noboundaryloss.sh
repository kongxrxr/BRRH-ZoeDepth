...
BASE_CHECKPOINT="${BASE_CHECKPOINT:-/home/kxr/zoedepth_kitti_beit_dav2gate_scale0p24_strong_skipnan_256x512_bs8_safe_checkpoints/ZoeDepthv1_10-Jun_19-32-d81d54b6af50_latest.pt}"
...
export SAVE_DIR="${SAVE_DIR:-/home/kxr/zoedepth_kitti_beit_dav2gate_scale0p24_noboundaryloss_skipnan_256x512_bs4_workers8_checkpoints}"
export LOG_FILE="${LOG_FILE:-${REPO_DIR}/logs/kitti_beit_dav2gate_scale0p24_noboundaryloss_skipnan_256x512_bs4_workers8_5ep.log}"
export DONE_FILE="${DONE_FILE:-${REPO_DIR}/logs/kitti_beit_dav2gate_scale0p24_noboundaryloss_skipnan_256x512_bs4_workers8_5ep.done}"
...
if [[ -f "${DONE_FILE}" ]]; then
  echo "===== SKIP scale0.24 no-boundary-loss; done marker exists: ${DONE_FILE} ====="
  exit 0
fi

echo "===== START scale0.24 no-boundary-loss epochs=${EPOCHS} checkpoint=${CHECKPOINT} $(date '+%F %T %Z') ====="
bash scripts/run_kitti_train_once.sh
echo "===== DONE scale0.24 no-boundary-loss $(date '+%F %T %Z') ====="
