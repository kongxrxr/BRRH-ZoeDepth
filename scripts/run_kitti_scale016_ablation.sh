...
run_variant() {
  ...
  export SAVE_DIR="/home/kxr/zoedepth_kitti_beit_dav2gate_scale0p16_${name}_skipnan_256x512_bs4_checkpoints"
  export LOG_FILE="${REPO_DIR}/logs/kitti_beit_dav2gate_scale0p16_${name}_skipnan_256x512_bs4_5ep.log"
  export DONE_FILE="${REPO_DIR}/logs/kitti_beit_dav2gate_scale0p16_${name}_skipnan_256x512_bs4_5ep.done"
  ...
  export CHECKPOINT="${BASE_CHECKPOINT}"
  ...
  export W_BOUNDARY_ALIGN="${w_align}"
  ...
  export FROZEN_DA_FUSION_SCALE=0.16
  export USE_FROZEN_DA_BOUNDARY_GATE="${use_gate}"
  export FROZEN_DA_MIN_GATE=0.05

  if [[ -f "${DONE_FILE}" ]]; then
    echo "===== SKIP ablation ${name}; done marker exists: ${DONE_FILE} ====="
    return 0
  fi

  echo "===== START ablation ${name} gate=${use_gate} w_align=${w_align} epochs=${EPOCHS} checkpoint=${CHECKPOINT} $(date '+%F %T %Z') ====="
  bash scripts/run_kitti_train_once.sh
  echo "===== DONE ablation ${name} $(date '+%F %T %Z') ====="
}

run_variant "nogate" 0 0.01
run_variant "noalign" 1 0
