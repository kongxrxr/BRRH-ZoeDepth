...
TRAIN_SESSION="${TRAIN_SESSION:-zoe_kitti_scale024_brrh_notemperature}"
CKPT_DIR="${CKPT_DIR:-/home/kxr/zoedepth_kitti_brrh_notemperature_scale0p24_256x512_bs4_workers8_5ep_checkpoints}"
OUT="${OUT:-${REPO_DIR}/logs/kitti_brrh_notemperature_scale0p24_bs4_workers8_5ep_hard_boundary_metrics.json}"
...
echo "===== HARD BOUNDARY EVAL BRRH no-temperature scale0.24 checkpoint=${ckpt} $(date '+%F %T %Z') ====="
"${PYTHON}" scripts/evaluate_kitti_boundary.py \
  ...
  --discontinuity_temperature_scale 1.0 \
...
echo "===== BRRH NO-TEMPERATURE SCALE 0.24 EVAL DONE $(date '+%F %T %Z') ====="
