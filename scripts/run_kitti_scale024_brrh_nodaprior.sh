...
export SAVE_DIR="${SAVE_DIR:-/home/kxr/zoedepth_kitti_brrh_nodaprior_scale0p24_256x512_bs4_workers8_5ep_checkpoints}"
export LOG_FILE="${LOG_FILE:-${REPO_DIR}/logs/kitti_brrh_nodaprior_scale0p24_256x512_bs4_workers8_5ep.log}"
export DONE_FILE="${DONE_FILE:-${REPO_DIR}/logs/kitti_brrh_nodaprior_scale0p24_256x512_bs4_workers8_5ep.done}"
...
export BOUNDARY_REFINE_USE_DA_PRIOR=0
...
export USE_FROZEN_DA_PRIOR=0
export FROZEN_DA_FEATURE_CHANNELS=8
export FROZEN_DA_INPUT_SIZE=384
export FROZEN_DA_FUSION_SCALE=0.0
export USE_FROZEN_DA_BOUNDARY_GATE=0
export FROZEN_DA_MIN_GATE=0.05

if [[ -f "${DONE_FILE}" ]]; then
  echo "===== SKIP BRRH no-DA-prior scale0.24; done marker exists: ${DONE_FILE} ====="
  exit 0
fi

echo "===== START BRRH no-DA-prior scale0.24 epochs=${EPOCHS} checkpoint=${CHECKPOINT} $(date '+%F %T %Z') ====="
bash scripts/run_kitti_train_once.sh
echo "===== DONE BRRH no-DA-prior scale0.24 $(date '+%F %T %Z') ====="
